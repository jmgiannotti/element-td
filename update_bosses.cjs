...
const BOSS_DEFS = [
    { key: 'enemy_boss_titan', baseName: 'boss_titan', label: 'Titán de Sillar' },
    { key: 'enemy_boss_dragon', baseName: 'boss_dragon', label: 'Ascua Primordial' },
    { key: 'enemy_boss_specter', baseName: 'boss_specter', label: 'Rey Velador' },
];
...
for (const b of BOSS_DEFS) {
    const asePath = path.join(__dirname, `${b.baseName}.ase`);
    const pngPath = path.join(__dirname, `${b.baseName}.png`);

    let result = null;
    let sourceFile = null;

    // Prefer .ase if it is newer or exists, otherwise .png
    if (fs.existsSync(asePath) && fs.existsSync(pngPath)) {
        const aseStat = fs.statSync(asePath);
        const pngStat = fs.statSync(pngPath);
        if (aseStat.mtimeMs >= pngStat.mtimeMs) {
            result = readAseprite(asePath);
            sourceFile = `${b.baseName}.ase`;
        } else {
            result = readPng(pngPath);
            sourceFile = `${b.baseName}.png`;
        }
    } else if (fs.existsSync(asePath)) {
        result = readAseprite(asePath);
        sourceFile = `${b.baseName}.ase`;
    ...
    if (result) {
        ...
        console.log(`  ✓ ${b.label} (${b.key}): cargado desde ${sourceFile} (${result.width}×${result.height})`);
    } else {
