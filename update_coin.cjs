/**
 * update_coin.cjs — coin.ase → Data URI PNG → CoinSprite.js
 *
 * Uso:
 *   node update_coin.cjs
 *
 * Parsea el .ase (RGBA 32bpp) componiendo las capas visibles y exporta el sprite
 * de la moneda recortado a sus píxeles útiles (17×17) en base64 para Phaser.
 */
...
console.log(`Parseando coin.ase: ${W}×${H}, ${colorDepth}bpp, ${numFrames} frame(s)`);
...
if (opaquePixels === 0) {
    console.error('ERROR: No se encontraron píxeles visibles en coin.ase');
    process.exit(1);
}
...
const content = `/**
 * Sprite de la moneda (oro) generado a partir de coin.ase.
 * Ejecutar 'node update_coin.cjs' para regenerar si se modifica el archivo aseprite.
 */
...
fs.writeFileSync(COIN_SPRITE_FILE, content, 'utf-8');
console.log(`✓ Generado ${path.relative(process.cwd(), COIN_SPRITE_FILE)} (${cropW}×${cropH})`);
